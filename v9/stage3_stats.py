...
def main():
    ...
    # Keep features with reasonable frequency (sparse but not dead)
    score = conditional_mean.clone()
    score[freq >= 0.15] = 0   # too common
    score[freq < 0.003] = 0   # too rare

    nonzero = (score > 0).sum().item()
    k = min(SWEEP_TOP_FEATURES, nonzero)
    top_indices = score.argsort(descending=True)[:k]
    print(f"  {nonzero} features with freq 0.3%-15%, selecting top {k}")

    # Load texts for high-activation mapping
    from datasets import load_dataset
    dataset = load_dataset("wikitext", "wikitext-103-raw-v1", split="train")
    texts = [s["text"] for s in dataset if len(s["text"].strip()) > 100][:ACT_NUM_SAMPLES]
