#!/usr/bin/env python3
"""Stage 1: Extract residual activations from GPT-2 to disk as fp16 binary."""
...
def main():
    ...
    model.eval()
    print(f"Model: {MODEL_NAME}, hidden={model.config.n_embd}, layers={model.config.n_layer}")

    print("Loading dataset (wikitext-103)...")
    dataset = load_dataset("wikitext", "wikitext-103-raw-v1", split="train")
    # Filter out empty lines and short texts
    texts = [s["text"] for s in dataset if len(s["text"].strip()) > 100]
    texts = texts[:ACT_NUM_SAMPLES]
