...
Logs to artifacts/label_log.txt since claude CLI output gets suppressed in nested contexts.
"""
...
def main():
    ...
    # Strip ALL claude-related env vars
    ...
    for i, (feat_idx, artifact_path) in enumerate(to_label):
        ...
        prompt = f"""You are analyzing activation features from a sparse autoencoder trained on TinyStories-33M.

Below is data for one feature. The gain sweeps show how generation changes when the feature is amplified (positive gain) or suppressed (negative gain).

{content[:4000]}

Provide:
1. SHORT NAME (2-5 words, lowercase, slash-separated like "emotion/sadness" or "topic/animals")
...
def build_meta_index(stats):
    ...
    lines = ["DREAMER FEATURE META-INDEX", "=" * 60, ""]
    ...
    lines.append("")
    lines.append(f"{'IDX':>6}  {'FREQ':>6}  {'SHORTNAME':<30}  DESCRIPTION")
    lines.append("-" * 90)
    for l in sorted(labels, key=lambda x: x["feature_idx"]):
        lines.append(f"{l['feature_idx']:>6}  {l['frequency']:>6.3f}  "
                     f"{l['shortname']:<30}  {l['description'][:70]}")

    META_INDEX_FILE.write_text("\n".join(lines))
    log(f"Meta-index written: {META_INDEX_FILE}")
